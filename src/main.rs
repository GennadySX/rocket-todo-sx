...
fn new(todo_form: Form<Todo>, conn: DbConn) -> Flash<Redirect> {
    ...
    if todo.description.is_empty() {
        Flash::error(Redirect::to("/"), "Ошибка при удаление таска.")
    } else if Task::insert(todo, &conn) {
        Flash::success(Redirect::to("/"), "Таск успешно создан.")
    } else {
        Flash::error(Redirect::to("/"), "Ошибка сервера.")
    }
}
...
fn toggle(id: i32, conn: DbConn) -> Result<Redirect, Template> {
    if Task::toggle_with_id(id, &conn) {
        ...
        Err(Template::render("index", &Context::err(&conn, "Таск не доступен.")))
    }
}
...
fn delete(id: i32, conn: DbConn) -> Result<Flash<Redirect>, Template> {
    if Task::delete_with_id(id, &conn) {
        Ok(Flash::success(Redirect::to("/"), "Таск удален."))
    } else {
        Err(Template::render("index", &Context::err(&conn, "Ошибка при удаление.")))
    }
}
...
fn run_db_migrations(rocket: Rocket) -> Result<Rocket, Rocket> {
    let conn = DbConn::get_one(&rocket).expect("Подключение к бд");
    match embedded_migrations::run(&*conn) {
        Ok(()) => Ok(rocket),
        Err(e) => {
            error!("Ошибка при соединение бд: {:?}", e);
            Err(rocket)
        }
    }
}
